...
# 更新城市字典，包含所有可能的城市ID
cities = {
    1: '北京', 2: '杭州', 3: '上海', 4: '广州', 5: '深圳',
    6: '成都', 7: '重庆', 8: '西安', 11: '厦门', 12: '丽江',
    13: '桂林', 14: '黄山', 15: '张家界', 16: '九寨沟', 17: '拉萨',
    51: '三亚', 33: '海口', 77: '青岛', 73: '厦门', 86: '大连', 36: '南京'
}
...
def recommend_items(user_id, n=10):
    ...
    for rank, cand in enumerate(scored_candidates[:n]):
        iid = cand['item_id']
        s_score = cand['score']
        item_data = item_info.get(iid, {'id': iid, 'name': f"景点{iid}", 'city': '其他城市', 'rating': 4.5, 'price': 0, 'category': '景点', 'city_id': 0})
        item_data['explain'] = f"NeuMF 置信度: {(s_score*100):.2f}%"
        final_recs.append(item_data)
        process.append(f" 推荐位[{rank+1}]: 景点{iid} ({item_data['city']}) (输出概率: {s_score:.4f})")
...
<body>
 ...
 <div class="search">
 <input type="text" id="userId" placeholder="请输入用户ID (尝试: 1000560)">
...
if __name__ == "__main__":
    print("\n" + "="*50)
    print("✅ 城市名称修复版 - 基于真实 PyTorch 权重的推荐系统启动成功！")
